...
The curation step performs the alignment of the hi-c reads against the draft assembly.
Files are produced for the hi-C viewer HiGlass and the assembly editor Pretext view.

### Manual curation

Download the curation files locally (`08_rapid_curation/{higlass,pretext}/`) and the report (`10_report/`).

Pretext view can be installed from [Github](https://github.com/sanger-tol/PretextView/releases) and
HiGlass can be run using docker:
```bash
docker pull higlass/higlass-docker:latest
docker run --detach \
    --publish 8989:80 \
    --volume 08_rapid_curation/higlass/data:/data \
    --volume 08_rapid_curation/higlass:/tmp \
...
Your Hi-C maps should have a strong signal (high density) along the diagonal and little off-diagonal (repetitive elements
will show fairly strong off-diagonal signals).

The aim is to rearrange sections in the assembly in Pretext view while using HiGlass to search for them since the resolution
in Pretext view is much lower. You should tag contigs as Haplotigs (not part of consensus, but are an alternate allele),
mark sex chromosomes (X,Y,W,Z), look for microchromosomes, and germline-restricted chromosomes, and so on. Contaminants may
also still be present, and can potentially be recognised through differences in coverage.
