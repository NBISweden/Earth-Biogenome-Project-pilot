...
```{python}
multiqc.get_plot("dtol","dtol-section").show() if "dtol" in multiqc.list_modules() else ""
```

### Genome Traits

```{python}
multiqc.get_plot("traits","traits-section").show() if "traits" in multiqc.list_modules() else ""
```
...
```{python}
multiqc.get_plot("quast","Assembly Statistics").show() if "quast" in multiqc.list_modules() else ""
```

```{python}
multiqc.get_plot("quast","Number of Contigs").show() if "quast" in multiqc.list_modules() else ""
```
...
```{python}
multiqc.get_plot("hifiasm","HiFiasm kmer graph").show() if "hifiasm" in multiqc.list_modules() else ""
```

#### Assembly gene space completeness

```{python}
multiqc.get_plot("busco","Lineage: bacteria_odb10").show() if "busco" in multiqc.list_modules() else ""
```
